...
class StudentProfile(models.Model):
    ...
    matric_number=models.CharField(max_length=30, null=True)
    department= models.CharField(choices=DEPARTMENT_CHOICE, max_length=50)
    state_of_origin=models.CharField(max_length=50,choices=STATE_OF_ORIGIN_CHOICES , null=True)
    ...
    student_file=models.FileField(upload_to='student-file/', null=True)
    student_reciept=models.FileField(upload_to='student-receipt/', null=True)
...
class StaffProfile(models.Model):
    ...
    def __str__(self):
        return f'({self.pk}). {self.firstname}\'s profile'
...
class ClearanceForm(models.Model):
    ...
    date_posted=models.DateField(auto_now_add=True, auto_now=False , null=True)
    ...
    def get_male_status(self):
        if self.boys_hostel_review == 'approved' and self.ptdf_wing_a_review == 'approved':
            return True
        return False

    def get_female_status(self):
        if self.noble_hostel_review == 'approved' and self.ptdf_wing_b_review == 'approved' and self.nddc_hostel_review == 'approved':
            return True
        return False

    def get_all_stats(self):
        if self.student_affair_review == 'approved' and self.sug_review == 'approved' and self.sport_officer_review == 'approved' and self.chief_liberian_review == 'approved' and self.hod_review =='approved' :
            return True

    def get_clearance_status(self):
        if self.sex== 'Male':
            if self.get_male_status() and self.get_all_stats() is True:
                return True
        else:
            if self.get_female_status() and self.get_all_stats() is True:
                return True