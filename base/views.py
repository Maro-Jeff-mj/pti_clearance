...
def clearance_certificate(request):
    try:
        get_qs= ClearanceForm.objects.get(posted_by= request.user)
        if get_qs.get_clearance_status():
            return render(request, 'clearance-certificate.html')
    except:
